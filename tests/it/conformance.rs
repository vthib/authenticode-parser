...
TEST_F(SignatureTest, SecondSignatureContent)
{
    ...
    assert_eq!(second_sig->version, 1);
    ...
    assert_eq!(second_sig->digest.len, 20);
    EXPECT_TRUE(std::memcmp(file_digest, second_sig->digest.data, 20) == 0);
    assert_eq!(second_sig->digest_alg, "sha1");

    assert_eq!(second_sig->verify_flags, AUTHENTICODE_VFY_VALID);

    //****************************//
    // Check SignerInfo structure //
    assert!(second_sig->signer);
    assert_eq!(second_sig->signer->digest_alg, "sha1");

    assert!(second_sig->signer->digest.data);
    ...
    assert_eq!(second_sig->signer->chain->count, 1);

    //******************************************//
    // Test all certificates of first signature //
    assert!(second_sig->certs);
    assert!(second_sig->certs->certs);
    ASSERT_EQ(second_sig->certs->count, 3);

    //**************************//
    // Check the 1. certificate //
    const Certificate *cert = second_sig->certs->certs[0];
    assert!(cert->sha1.data);
    ...
    assert_eq!(cert->not_after, 1533859199);
    assert_eq!(cert->not_before, 1502323200);
    assert_eq!(
        cert->key,
        ...
        "HLEtEekQIDAQAB");
    assert_eq!(cert->serial, "06");
    assert_eq!(cert->sig_alg, "sha1WithRSAEncryption");
    assert_eq!(cert->sig_alg_oid, "1.2.840.113549.1.1.5");
    assert_eq!(cert->key_alg, "rsaEncryption");
    ...
    assert_eq!(cert->not_after, 1925942399);
    assert_eq!(cert->not_before, 1452556800);
    assert_eq!(
        cert->key,
    ...
    assert_eq!(cert->serial, "7b:05:b1:d4:49:68:51:44:f7:c9:89:d2:9c:19:9d:12");
    assert_eq!(cert->sig_alg, "sha256WithRSAEncryption");
    assert_eq!(cert->sig_alg_oid, "1.2.840.113549.1.1.11");
    assert_eq!(cert->key_alg, "rsaEncryption");
    ...
    assert_eq!(
        ...
        "/C=US/O=Symantec Corporation/OU=Symantec Trust Network/CN=Symantec SHA256 TimeStamping "
        "CA");
    assert_eq!(cert->not_after, 1868918399);
    assert_eq!(cert->not_before, 1513987200);
    assert_eq!(
        cert->key,
        ...
        "LFtG/l77CKmwIDAQAB");
    assert_eq!(cert->serial, "7b:d4:e5:af:ba:cc:07:3f:a1:01:23:04:22:41:4d:12");
    assert_eq!(cert->sig_alg, "sha256WithRSAEncryption");
    assert_eq!(cert->sig_alg_oid, "1.2.840.113549.1.1.11");
    assert_eq!(cert->key_alg, "rsaEncryption");
    ...
    //*******************************************//
    // Test the first signature countersignature //
    assert!(second_sig->countersigs);
    assert!(second_sig->countersigs->counters);
    ASSERT_EQ(second_sig->countersigs->count, 1);

    const Countersignature *countersig = second_sig->countersigs->counters[0];

    assert_eq!(countersig->verify_flags, COUNTERSIGNATURE_VFY_VALID);
    assert_eq!(countersig->digest_alg, "sha1");
    assert_eq!(countersig->sign_time, 1527779086);
...
TEST_F(SignatureTest, ThirdSignatureContent)
{
    ...
    assert_eq!(third_sig->digest.len, 32);
    EXPECT_TRUE(std::memcmp(file_digest, third_sig->digest.data, 32) == 0);
    assert_eq!(third_sig->digest_alg, "sha256");

    assert_eq!(third_sig->verify_flags, AUTHENTICODE_VFY_VALID);

    //****************************//
    // Check SignerInfo structure //
    assert!(third_sig->signer);
    assert_eq!(third_sig->signer->digest_alg, "sha256");

    assert!(third_sig->signer->digest.data);
    ...
    assert_eq!(third_sig->signer->chain->count, 2);
    ...
    assert_eq!(
        ...
        "Signing CA");
    assert_eq!(cert->not_after, 1579046399);
    assert_eq!(cert->not_before, 1513036800);
    assert_eq!(
        cert->key,
    ...
    assert_eq!(cert->serial, "38:bf:a6:1b:82:b8:0f:60:57:15:e4:8a:a1:0d:e1:53");
    assert_eq!(cert->sig_alg, "sha256WithRSAEncryption");
    assert_eq!(cert->sig_alg_oid, "1.2.840.113549.1.1.11");
    assert_eq!(cert->key_alg, "rsaEncryption");
    ...
    assert_eq!(
        ...
        "authorized use only/CN=VeriSign Class 3 Public Primary Certification Authority - G5");
    assert_eq!(cert->not_after, 1702166399);
    assert_eq!(cert->not_before, 1386633600);
    assert_eq!(
        cert->key,
    ...
    assert_eq!(cert->serial, "3d:78:d7:f9:76:49:60:b2:61:7d:f4:f0:1e:ca:86:2a");
    assert_eq!(cert->sig_alg, "sha256WithRSAEncryption");
    assert_eq!(cert->sig_alg_oid, "1.2.840.113549.1.1.11");
    assert_eq!(cert->key_alg, "rsaEncryption");

    assert!(third_sig->certs);
    assert!(third_sig->certs->certs);
    ASSERT_EQ(third_sig->certs->count, 4);

    //*******************************************//
    // Test the first signature countersignature //
    assert!(third_sig->countersigs);
    assert!(third_sig->countersigs->counters);
    ASSERT_EQ(third_sig->countersigs->count, 1);

    const Countersignature *countersig = third_sig->countersigs->counters[0];

    assert_eq!(countersig->verify_flags, COUNTERSIGNATURE_VFY_VALID);
    assert_eq!(countersig->digest_alg, "sha256");
    assert_eq!(countersig->sign_time, 1527779085);
...
TEST(PefileTest, ResultOverview)
{
    ...
    const Authenticode *sig = auth->signatures[0];
    {
        ...
        assert_eq!(sig->digest_alg, "sha1");

        assert_eq!(sig->certs->certs[0]->sig_alg_oid, "1.2.840.113549.1.1.5");
        assert_eq!(sig->certs->certs[0]->sig_alg, "sha1WithRSAEncryption");
